...
const CuentasPorPagarPage = () => {
  ...
  const loadCuentas = async () => {
    try {
      setLoading(true)
      const data = await cuentasPagarService.getAllCuentasPagar()
      setCuentas(data)
    } catch (error) {
  ...
  const cuentasFiltradas = cuentas.filter((cuenta) => {
    const matchBusqueda =
      cuenta.ordenes_compra?.numero_orden?.toLowerCase().includes(busqueda.toLowerCase()) ||
      cuenta.suplidores?.nombre?.toLowerCase().includes(busqueda.toLowerCase())
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-muted/30 p-6">
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-balance">Cuentas por Pagar</h1>
              <p className="text-muted-foreground">Gestión de obligaciones con suplidores</p>
            </div>
            <Button asChild>
              <Link href="/suplidores/reportes">
          ...
          <div className="grid gap-6 md:grid-cols-4 mb-6">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-destructive">RD$ {totalPendiente.toLocaleString()}</div>
                <p className="text-xs text-muted-foreground mt-1">{cuentasFiltradas.length} órdenes</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {cuentasFiltradas.filter((c) => c.estado === "vencida").length} órdenes
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {cuentasFiltradas.filter((c) => c.estado === "pendiente").length} órdenes
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {cuentasFiltradas.filter((c) => c.estado === "parcial").length} órdenes
                </p>
              </CardContent>
            </Card>
          </div>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-end">
                <div className="flex-1 space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      id="busqueda"
                      placeholder="Buscar por orden o suplidor..."
                      className="pl-10"
                      value={busqueda}
                      ...
                      cuentasFiltradas.map((cuenta) => (
                        <TableRow key={cuenta.id}>
                          <TableCell className="font-mono text-sm">
                            {cuenta.ordenes_compra?.numero_orden || "N/A"}
                          </TableCell>
                          <TableCell className="font-medium">{cuenta.suplidores?.nombre || "N/A"}</TableCell>
                          <TableCell>
                            {cuenta.fecha_orden ? new Date(cuenta.fecha_orden).toLocaleDateString("es-DO") : "N/A"}
                          </TableCell>
                          <TableCell>
